...
def train_one_epoch(
    ...
    for step, (images, labels, pos, board) in enumerate(progress_bar):
        ...
        if rank == 0 and (step + 1) % log_interval == 0:
            ...
            if save_images:
                logs['Train Images'] = plot_trajectory(model, images[:12], pos[:12], board[:12], n_samples=5)
...
def main():
    ...
    # shifted to new data
    train_dataset = OrigPlank3("/cifs/data/tserre_lrs/projects/projects/prj_vis_sim/plankdatasets/originalv1/train", train = True, transform = transform, noise = noise, norm = normalization)
    test_dataset = OrigPlank3("/cifs/data/tserre_lrs/projects/projects/prj_vis_sim/plankdatasets/originalv1/test", train = False, transform = transform, noise = noise, norm = normalization)
    ...
    if args.vae_training:
        modelname = "cvae_grid"
        model = CVAE(board_dim = 105, trajectory_dim = 16, latent_dim = latent_size).to(device)
    else:
