...
def compute_and_save_ig(model, dataloader, device='cuda', explainer = "ig"):
    ...
    for step, (images, labels, pos, board) in enumerate(progress_bar):
        ...
        if explainer == "ig":
            attributions = exp.attribute(images, target=[(31, int(x[31])) for x in pos], n_steps=50, internal_batch_size = 32)
        else:
            attributions = exp.attribute(images, target=(), sliding_window_shapes=(1,30,40))
...
def main():
    ...
    batch_size = 32
    latent_size = 4
    test_dataset = OrigPlank5("/cifs/data/tserre_lrs/projects/projects/prj_vis_sim/plankdatasets/singbaskv3/test", train = False, is_val = False, transform = transform, noise = False, norm = False)
