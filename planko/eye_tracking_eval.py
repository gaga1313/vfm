...
def plot_trajectory(model, images, eye_images, labels, n_samples=1):
    ...
    for i, img in enumerate(images):
        ...
        for traj in predicted_trajectories[i, :]:
            ...
            label = (
                f"Basket: {labels[i]}" if first_traj else "_"
            )  # Assign an actual label only once
    ...
    for i, img in enumerate(images):
        ...
        for j, (x, y) in enumerate(mean_traj):
            ...
            axes[0].scatter(
                x * 11.2 + 111.5,
                y * 11.2 + 112,
                s=max(max(circle_size), 5),
                alpha=max(max(alpha_value), 0.25),
                color="#396ca3",
            )
...
def test(
    ...
    with torch.no_grad():
        for step, (board_images, eye_images, labels) in enumerate(progress_bar):
            ...
            logs = {}
            logs["Eye_Track Images"] = plot_variance_heatmap(
                model, board_images, eye_images, n_samples=50
            )
...
def main():
    ...
    test_dataset = EyeTrackData(
        "/files22_lrsresearch/CLPS_Serre_Lab/projects/prj_vis_sim/s20_board_eye_tracking",
        transform=transform,
    )
    ...
    if args.vae_test:
        ...
        load_path = os.path.join(
            "pretrained_models", modelname, "trial_2_best_model.pth"
        )
        modelname = (
            "S20_eye_tracking" + str(latent_size) + "_" + str(kld) + "_8_" + modelname
        )
