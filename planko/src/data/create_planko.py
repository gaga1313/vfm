...
def create_world(
    tx, ty, sx, sy, angles, n_planks=10, basket_draw=1, import_world=False
):
    ...
    if import_world:
        ...
        for i in range(n_planks):
            ...
            box = b2PolygonShape(
                box=(plank_length, 4, (plank_pos_x, plank_pos_y), plank_angle * b2_pi)
            )
            world.CreateBody(shapes=box)

    circle = b2CircleShape(
        pos=(INITIA_BALL_X, INITIA_BALL_Y * yscalar + bias), radius=BALLRADIUS
    )
    ballfixture = b2FixtureDef(shape=circle, density=0.00005, restitution=0.27)
    ball = world.CreateDynamicBody(
        fixtures=ballfixture,
        bullet=True,
        userData={"type": "ball", "hit": False, "hit_basket": 0, "trajectory": []},
    )

    print(
        f"Initial ball position: {(INITIA_BALL_X, INITIA_BALL_Y * yscalar + bias)}"
...
def my_draw_circle(circle, body, fixture):
    ...
    if not body.userData.get(
        "hit", False
    ):  # Only record if ball has not hit the basket
        ball_positions.append(position)
    # for pos in ball_positions:
    #     pygame.draw.circle(screen, (205, 200, 200), [int(x) for x in pos], BALLRADIUS)
    pygame.draw.circle(
        screen, colors["ballColor"], [int(x) for x in position], int(circle.radius)
    )
    ...
    if ball.userData["hit"]:
        ...
        for j in range(1, len(post_hit_positions)):
            total_distance += calculate_distance(
                post_hit_positions[j - 1], post_hit_positions[j]
            )
        print(
            f"Total actual distance traveled after hitting the basket: {total_distance}"
        )
...
if __name__ == "__main__":
    ...
    with open("../../data/combined_final_infor.csv", mode="r") as file:
        ...
        for lines in csvFile:
            ...
            pygame.init()
            world, ball = create_world(
                tx, ty, sx, sy, angles, n_planks, baskettodraw, True
            )
