...
class EyeTrackData(Dataset):
    def __init__(
        ...
        t0 = time.time()
        self.eye_file_list = [os.path.join(self.path, f) for f in os.listdir(self.path) if os.path.isfile(os.path.join(self.path, f))]
        self.board_file_list = [os.path.join(self.board_path, f) for f in os.listdir(self.board_path) if os.path.isfile(os.path.join(self.board_path, f))]
    ...
    def __getitem__(self, index: int):
        ...
        file_name = file_name.replace("/media/data_cifs_lrs", "/files22_lrsresearch/CLPS_Serre_Lab")
