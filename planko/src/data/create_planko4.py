...
class OrigPlank4(Dataset):
    def __init__(
        self, path: ValueNode, train: bool, is_val:bool, transform = None, noise = False, norm = False, **kwargs
    ):
        super().__init__()
        # self.cfg = cfg
        self.path = path
        ...
        n = 0.9
        self.train_samples = int(len(self.file_list)*n)

        if train:
            self.file_list = self.file_list[:self.train_samples]
        if is_val:
            self.file_list = self.file_list[self.train_samples:]
        # if not self.train:
    ...
    def __getitem__(self, index: int):
        ...
        xbins = np.arange(-28, 30, 1)
        binned_indices = np.digitize(x_positions, xbins) - 1
        x_new_positions = np.clip(binned_indices, 0, len(xbins) - 2)

        ybins = np.arange(-8, 10, 1)
        binned_indices = np.digitize(y_positions, ybins) - 1
        y_new_positions = np.clip(binned_indices, 0, len(ybins) - 2)


        labels = 56 * y_new_positions + x_new_positions
        ...
        positions = np.array(labels[:16], dtype = np.float32)
        
        # positions = np.zeros((16, 896), dtype=np.float32)  # Initialize with zeros
        # positions[np.arange(16), labels] = 1
        
        # positions = np.zeros((896), dtype = np.float32)
        # positions[labels] = 1

        # if sum(positions) < 16:
        #     print(sum(positions), labels)
...
#     train_dataset = OrigPlank3("/cifs/data/tserre_lrs/projects/projects/prj_vis_sim/plankdatasets/originalv1/train", train = True, is_val = False, transform = transform, noise = True)
#     val_dataset = OrigPlank3("/cifs/data/tserre_lrs/projects/projects/prj_vis_sim/plankdatasets/originalv1/train", train = False, is_val = True, transform = transform, noise = True)
#     import ipdb;ipdb.set_trace()
...
#     for img, label, pos, board in progress_bar:
#         import ipdb; ipdb.set_trace()
#         x = 0
