...
def plot_trajectory(model, images, pos, n_samples = 1):
    predicted_trajectories = model.module.inference(images, n_samples = n_samples)
    images = images * 255
    ...
    for i, img in enumerate(images):
        ...
        curr_pos = np.array([[x_bin_coor[x], y_bin_coor[ybin[idx]]] for idx, x in enumerate(xbin)])
...
def train_one_epoch(
    ...
    for step, (images, labels, pos, board) in enumerate(progress_bar):
        ...
        if rank == 0 and (step + 1) % log_interval == 0:
            ...
            if args.vae_training:
                logs = {
                        "train_cce_loss": cce_loss.item(), "epoch": epoch + 1, 
            ...
            if save_images and (step + 1) % 100 == 0:
                logs['Train Images'] = plot_trajectory(model, images[:12], pos[:12], n_samples=5)
        ...
        if args.vae_training:
            progress_bar.set_postfix(
                {"Train CCE Loss": cce_loss.item(), "Train KLD": loss_kld.item(), "Train Loss": batch_loss}
            )
...
def test(
    ...
    with torch.no_grad():
        for step, (images, labels, pos, board) in enumerate(progress_bar):
            ...
            if rank == 0 and (step + 1) % log_interval == 0:
                ...
                if args.vae_training:
                    logs = {
                            "test_cce_loss": cce_loss.item(), "epoch": epoch + 1, 
                            "test_kld_loss": loss_kld.item(), "test_loss": batch_loss
                        }
                else: 
                    logs = {
                ...
                if save_images and (step + 1) % 10 == 0:
                    logs['Validation Images'] = plot_trajectory(model, images[:12], pos[:12], n_samples=1)
            ...
            if args.vae_training:
                progress_bar.set_postfix(
                    {"Test Loss": batch_loss,  "Test KLD": loss_kld.item(), "Test CCE Loss": cce_loss.item()}
                )
            else:
                progress_bar.set_postfix(
    ...
    #     if args.vae_training:
    #         wandb.log(
    #             {"test_cce_loss": avg_cce_loss, "test_kld_loss": avg_kld_loss, "test_loss": avg_loss},
    #             step = global_step
    #         )
    #     else:
    #         wandb.log({"test_cce_loss": avg_cce_loss},
    #                 step = global_step
    #         )
    return avg_loss, global_step
...
def main():
    ...
    # shifted to new data
    train_dataset = OrigPlank5("/cifs/data/tserre_lrs/projects/projects/prj_vis_sim/plankdatasets/singbaskv3/train", train = True, is_val = False, transform = transform, noise = noise, norm = normalization)
    val_dataset = OrigPlank5("/cifs/data/tserre_lrs/projects/projects/prj_vis_sim/plankdatasets/singbaskv3/train", train = False, is_val = True, transform = transform, noise = noise, norm = normalization)
    # test_dataset = OrigPlank3("/cifs/data/tserre_lrs/projects/projects/prj_vis_sim/plankdatasets/originalv1/test", train = False, is_val = False, transform = transform, noise = noise, norm = normalization)
    ...
    test_dataloader = DataLoader(val_dataset, batch_size=batch_size, sampler=test_sampler)
    ...
    if rank == 0:
        ...
        meta_data = {
            ...
            "is_vae_training": args.vae_training,
            "dataset":"singbaskv3"
        }
    ...
    best_loss = float("inf")
    save_path = os.path.join("pretrained_models", modelname)
    dataset_name = "sbk_planko_trajectory_"
    ...
    for e in range(epochs):
        ...
        if rank == 0:
            print(
                f"Epoch {e+1}/{epochs} - Train CCE Loss: {train_loss:.4f}"
            )
