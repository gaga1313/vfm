        ...
        if rank == 0 and (step + 1) % log_interval == 0:
            ...
            if save_images and (step + 1) % 100 == 0:
                logs["Train Images"] = plot_trajectory(
                    model, images[:12], pos[:12], n_samples=5
                )
        ...
        for step, (images, labels, pos, board) in enumerate(progress_bar):
            ...
            if rank == 0 and (step + 1) % log_interval == 0:
                ...
                if save_images and (step + 1) % 10 == 0:
                    logs["Validation Images"] = plot_trajectory(
...
def main():
    ...
    # shifted to new data
    train_dataset = OrigPlank5(
        "/cifs/data/tserre_lrs/projects/projects/prj_vis_sim/plankdatasets/singbaskv3/train",
        train=True,
        is_val=False,
        transform=transform,
        noise=noise,
    ...
    test_dataloader = DataLoader(
        val_dataset, batch_size=batch_size, sampler=test_sampler
    )
