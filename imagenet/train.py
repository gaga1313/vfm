...
def load_data(data_dir, split = "train", batch_size = 32, world_size = None, rank = None):
    if not world_size or not rank:
        assert('Please pass world_size and rank arguments!')
...
def main():
    ...
    for epoch in range(num_epochs):
        train_loss, train_acc, global_step = train_one_epoch(
            model, vae, train_loader, criterion, optimizer,
            epoch, device, rank, global_step=global_step
        )
        ...
        val_loss, val_acc = evaluate(
            model, vae, val_loader, criterion,
            device, rank, global_step
        )
        ...
        if rank == 0:
            ...
            if val_acc > best_val_acc:
                ...
                torch.save(model.module.state_dict(), os.path.join(save_path, run_id))
